...
//The purpose of providing prototypes for the funtion in this code module 
//is to declare the function signature before it is used. This lets the compiler
//know the information needed for this program and ensures everything wotks.
void  usage(char *);
...
void  word_print(char *str){
    ...
    for (int i = 0; i <= last_char_idx; i++){
        ...
        if (word_start) {
            ...
            if (isspace(str[i]) || i == last_char_idx) {
                printf(" (%d)\n", wlen);
                word_start = false;
            }
        }
...
int main(int argc, char *argv[]){
    ...
    char opt;   
    int wc;            //used to capture user option from cmd line
    ...
    switch (opt){
        ...
        case 'r':
            ...
            //the string is reversed in place because the algorithm swaps the characters
            //of the directly in the memory. 
            break;
        case 'w':
            printf("Word Print\n----------\n");
        ...
        //the purpose of the default option here is to handle cases where the user
        //gives an invalid option, this will display a error message
        default:
            usage(argv[0]);
    ...
    //The break statements in each case help prevent fall-through, when many cases 
    //execute by accident. If we forgot the breal it can cause unexpected behavior,
    //like running the wrong piece of code. 
}
